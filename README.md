...
> **v5.0 업데이트**: **Supabase 실제 연동 시작!** 이메일 매직링크 로그인이 추가됐고(상단 네비 아래), 로그인 상태면 묵상 기록이 `entries` 표에도 저장돼요(로그인 안 해도 예전처럼 로컬 저장은 그대로 동작 — 하이브리드 방식). 브라우저에서 로그인 테스트해보시고 문제 있으면 알려주세요 — 이 코드를 만든 환경에서는 Supabase 서버 접속이 막혀있어서 실제 연결 검증은 못 했어요. 상세는 `SOW_구조설계.md` 21절 참고.
