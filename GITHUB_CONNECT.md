...
**v7.0부터 기본 원칙**: 전체 프로젝트를 통째로 다시 압축하는 대신, **그때그때 실제로 바뀐 파일/폴더만** 받아서 반영해요. 매번 전체 zip을 주고받으면 크레딧/데이터가 불필요하게 많이 들어가요 — 예를 들어 CSS 한 줄만 고쳤는데 사진이 잔뜩 든 폴더까지 통째로 다시 압축할 필요는 없으니까요.

> **실제로 뭐가 얼마나 차이나냐면**: 전체 zip은 약 1.6MB(대부분 앱 아이콘 이미지 때문)인데, 바뀐 파일 몇 개만 묶으면 보통 수십 KB — 다운로드 용량 기준으로 100배 넘게 차이나요. Claude 쪽 작업량은 "zip으로 묶는 것" 자체보다는, **관련 문서 전부를 매번 다시 훑어보지 않고 바뀐 부분에만 집중하는 것**에서 절약이 커요.

### 방법 A. 바뀐 파일이 1~2개뿐일 때 — GitHub 웹에서 직접 편집
1. `https://github.com/sowbible/seed-of-word-v2`에서 그 파일 찾아 클릭 (예: `app/css/shell.css`)
2. 연필 아이콘(✏️) 클릭 → 내용 전체 선택(Ctrl+A) → 지우고 새 내용 붙여넣기
3. **"Commit changes..."** → **"Commit changes"**

새 파일을 추가하는 거면, 저장소 화면에서 **"Add file" → "Create new file"**로 만들면 돼요.

### 방법 B. 바뀐 파일/폴더가 여러 개일 때 — 그 부분만 압축해서 GitHub Desktop으로
1. 바뀐 파일/폴더만 담은 작은 zip을 받음 (예: `content/korean/` 폴더 전체, 또는 `app/js/shell.js` + `app/css/shell.css` 두 개만)
2. 압축 풀기
3. GitHub Desktop → **Repository → Show in Explorer**로 저장소 폴더 열기
4. 압축 푼 내용을 저장소 폴더의 **같은 위치에** 복사해서 붙여넣기 → "바꾸기" 선택
5. GitHub Desktop에서 **바뀐 파일만** 목록에 뜨는지 확인 → Summary 쓰고 **Commit to main** → **Push origin**

전체 zip이 필요한 경우는 아주 드물게(예: 처음 시작할 때, 또는 정말 대부분의 파일이 바뀌었을 때)만 쓸게요. 특정 파일이 정확히 뭐가 바뀌었는지 헷갈리면 언제든 "이번엔 뭐가 바뀌었어?"라고 물어보시면 정확히 알려드릴게요.

---

### (참고) 예전 방식 — 전체 zip으로 통째로 갱신하던 순서

처음엔 저장소가 비어있어서 "Clone"부터 했지만, **한 번 clone해두면 그 다음부터는 훨씬 짧아요.** 전체 zip을 받은 경우엔 이 순서예요.
...
## 검증은 이렇게 하면 돼요 (3단계, 필요한 만큼만)

**Push하기 전, 제가(Claude) 하는 것 — 항상, 비용 거의 안 듦:**
- JS 문법 검사(`node --check`), JSON 형식 검사, CSS 중괄호 짝 확인
- 이건 코드에 "실수로 오타 남" 같은 걸 잡아내는 최소한의 확인이에요. 매번 자동으로 해요.

**구조가 크게 바뀔 때만 — 로컬 서버 켜서 실제 경로 확인:**
- 파일 위치를 옮기거나(예: v7.0의 국어 폴더 구조 변경), 페이지 이동 로직을 바꾸는 것처럼 "어디서 뭘 불러오는지"가 바뀌는 변경일 때만, 로컬 서버 켜서 그 경로들이 실제로 응답하는지 확인해요. 사소한 글자 크기/색 조정 같은 건 이 단계 생략해요.

**실제 화면 확인은 사용자님 몫 — 이게 사실 제일 정확해요:**
- 저는 이 환경에서 헤드리스 브라우저 설치가 막혀있어서, **제가 직접 화면을 보고 확인할 방법이 없어요.** 그래서 진짜 브라우저에서 어떻게 보이는지는 사용자님이 확인해주시는 게 유일하고 가장 정확한 방법이에요.
- 배포된 Netlify 사이트는 Push하면 자동으로 반영되니까, Push 후 **강력 새로고침(Ctrl+Shift+R)** 하고 아래 정도만 훑어보시면 충분해요:
  - CSS/디자인만 바뀜 → 그 화면 눈으로 확인 (색, 크기, 배치)
  - 콘텐츠(JSON) 하나 추가/수정 → 그 페이지 열어서 내용이 잘 나오는지
  - 구조/로직(JS)이 바뀜 → 관련된 화면 몇 개 클릭해보기 (예: 메뉴 눌러보기, 코스 바꿔보기, 로그인 시도)

**만약 뭔가 깨지면:**
- GitHub Desktop의 **History** 탭에서 이전 커밋을 보고 되돌릴 수 있어요(git이 다 기록해두니까). 심하게 꼬이면 "이전 커밋으로 되돌리고 싶어"라고 말씀해주시면 그 방법도 알려드릴게요.
