...
> ✅ **진행 상황(2026-08-29)**: 프로젝트 생성 완료(`seddofword2`, Seoul), SQL 실행 완료, 앱 코드 연결 완료(`SOW_구조설계.md` 21절). 아래는 그 과정에서 실제로 진행한 순서를 그대로 남겨둔 기록이자, 나중에 다른 환경에 다시 세팅할 때 참고할 가이드예요.
...
> ⚠️ **꼭 복사 버튼(📋)으로만 복사하세요, 절대 손으로 옮겨 타이핑하지 마세요.** Project URL의 랜덤 문자열(`hzjfwqawdncdjtezcooa` 같은 것)은 사람이 눈으로 보고 옮겨적으면 글자 하나 빠뜨리거나 더 넣기 정말 쉬워요 — 실제로 이 프로젝트에서도 한 글자가 더 들어가서 로그인이 "Failed to fetch"로 계속 실패했던 적이 있어요(`SOW_구조설계.md` 16-5-1절 참고). 복사 버튼 누르고 그대로 붙여넣기만 하면 이런 실수가 아예 안 생겨요.
